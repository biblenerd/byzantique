...
const BOOK = {
  ...
  Gen: 'GEN', Exod: 'EXO', Num: 'NUM', Deut: 'DEU', Josh: 'JOS', Judges: 'JDG',
...
function refLabel(ranges) {
  ...
  for (const r of ranges) {
    ...
    if (!g) groups.push((g = { code: r.code, parts: [] }));
    if (r.ev === WHOLE) g.parts.push(`${r.sc}`);
    else if (r.sc === r.ec) g.parts.push(r.sv === r.ev ? `${r.sc}:${r.sv}` : `${r.sc}:${r.sv}-${r.ev}`);
    else g.parts.push(`${r.sc}:${r.sv}-${r.ec}:${r.ev}`);
  }
  return groups
    .map((g) => {
      const meta = bookByCode(g.code);
      return `${meta ? bookLabel(meta) : g.code} ${g.parts.join(', ')}`;
    })
    .join('; ');
}
...
function build() {
  ...
  for (const r of readings) {
    ...
    const meta = bookByCode(first.code);
    if (!meta) { unresolved.add(per.sdisplay); continue; }
    const href = `/${meta.testament}/${meta.slug}/${first.sc}${first.ev === WHOLE ? '' : `#v${first.sv}`}`;

    // place the chip on every (book, chapter) the reading touches
    const seen = new Set();
